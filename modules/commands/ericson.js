  ...
  try {
    ...
    if (query) {
      ...
      if (response.data && response.data.message) {
        ...
        console.log(`Sent 🎁 Ericson 's response to the user`);
      } else {
        throw new Error(`Invalid or missing response from 🎁 Ericson  API`);
      }

      await api.unsendMessage(processingMessage.messageID);
    }
  } catch (error) {
    console.error(`❌ | Failed to get 🎁 Ericson 's response: ${error.message}`);
